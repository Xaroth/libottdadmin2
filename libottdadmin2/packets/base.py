...
def check_length(value, max_length, name="Value", include_null=True):
    ...
    if length > max_length:  # pragma: no cover
        raise ValueError(
            "%s may not be longer than %d characters (%d)" % (name, max_length, length)
        )
    return value
...
def check_tuple_length(value, min_length=0, max_length=0, name="Value"):
    ...
    if length < min_length:
        raise ValueError(
            "%s must be longer than %d items (%d)" % (name, min_length, length)
        )
    elif length > max_length:
        raise ValueError(
            "%s must be smaller than %d items (%d)" % (name, max_length, length)
        )
    return value
...
class Packet:
    ...
    def register(klass):
        ...
        if not getattr(klass, "data", None):
            fields = [
                (x, Any) if not (isinstance(x, (list, tuple)) and len(x) == 2) else x
    ...
    def _read_batch(self, fmt) -> Tuple[Any, ...]:
        ...
        if obj.size > size_remaining:
            raise PacketExhaustedError(
                "%d bytes requested, but only %d available" % (obj.size, size_remaining)
            )
